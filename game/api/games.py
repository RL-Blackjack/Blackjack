...
def create_game(몸체: NewGameIn,
    ...
    # 왜 여기서인가: 라운드를 여는 길목이다. 옛 규칙의 열린 라운드가 남아 있으면
    #   아래 확인이 영원히 409를 낸다.
    옛라운드정리(session, 사용자.id)
...
def finish_game(game_id: GameId,
    ...
    게임 = 게임찾기(session, game_id, 사용자)
    # 왜 여기서도 정리하는가: 규칙이 바뀐 옛 게임은 열린 라운드를 둘 수 없어 끝내기가
    #   open_round 로 막힌다. 먼저 치워야 사용자가 그 게임을 스스로 닫을 수 있다.
    옛라운드정리(session, 사용자.id)
