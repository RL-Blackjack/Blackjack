...
def test_제어문자와_보이지_않는_문자는_거부된다(클라):
    ...
    # 왜 U+1160·U+FFA0·U+E000·U+FDD0도 보내는가: 한글 채움 문자 둘은 범주가
    #     Lo(글자)라 범주 검사로 안 걸리고, 사용자 정의 영역(Co)과 영원히
    #     미할당인 비문자(Cn)는 화면에 아무것도 아닌 것으로 나온다. 문단
    #     구분자 U+2029는 가운데에 끼면 strip이 떼지 못해 그대로 저장된다.
